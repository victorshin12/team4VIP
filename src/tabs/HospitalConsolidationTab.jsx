...
export default function HospitalConsolidationTab() {
  ...
  const [matchHcrisNameSource, setMatchHcrisNameSource] = useState('') // which HCRIS column we matched on
  const [matchHcrisIncomeSource, setMatchHcrisIncomeSource] = useState('') // which HCRIS column we used for income
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const resp = await fetch('/Hospital_CHOW_2026.01.csv', { cache: 'no-store' })
